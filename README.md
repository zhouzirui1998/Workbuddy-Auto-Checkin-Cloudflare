...
- 读取单个或全部账号的剩余积分、总积分及各批次到期时间
- 账号卡用一条按剩余积分比例分段的横线展示所有可用积分；每段对应一个到期时间，点击“查看到期批次”可查看具体积分和北京时间。官方未返回到期时间的积分单独标为“到期时间未知”
...
从旧版本更新后，旧积分缓存只有合计值和最近到期时间；请在管理后台点击“刷新全部积分”，成功读取后才会出现按批次分段的横线。横线的长度比例按各批次**剩余积分**计算，相同到期时间会合并；灰色段表示官方没有提供到期时间，不能据此判断为永久有效。此展示方式参考 [WorkDaddy 的积分批次设计](https://github.com/babygoton/WorkDaddy)。
